...
export async function generateVideo(args: GenerateVideoArgs): Promise<void> {
  ...
  const spin = spinner('Submitting video job…');
  let job;
  try {
  ...
  const waitSpin = spinner('Waiting for video to complete (this may take a few minutes)…');
  try {
    const done = await client.waitForJob(job.mediaJobId, { timeout: 600_000 });
